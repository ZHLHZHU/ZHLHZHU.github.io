...
靠猜+重刷来试错太慢（每轮编译+OTA 要 3 分钟）。我换了个思路：在一份固件里同时内置 4 种候选编码，加一条诊断命令让它对每种编码在配对信道上扫频统计 ACK 数——让接收器来投票哪个是对的。一次刷入，跑一遍，编码 1 拿到了 ACK，其余全 0，一锤定音。
...
正确方向是**从源头就不让它断**：用 embassy 的 `select(read_packet, Timer::after(8ms))`，在等命令的间隙持续发保活包，跟真实键盘一样一直维持连接。
...
Scroll Lock 这个按键和对应的指示灯应该是键盘上最摸鱼的部分了，将他们利用起来作为键盘设备切换和指示：
...
日常使用就是：键盘插在树莓派上，按 Scroll Lock 切目标，打字即达。目标电脑只看到一个"罗技键盘"，不需要装任何东西。

开机自启（systemd），切换一下不到 300ms，输入延迟 3-8ms 人完全无感。CPU 占用 0%（epoll 阻塞，不轮询）。
